...
(** ameliorer l'algo de placement a l'aide de la recherche dichotomique, en renvoyant l'indice de la fonction du cfg concerné puis ensuite l'endroit ou le noeud devrait se placer**)
  ...
  if value.noeud = ne  
    then (
      let (b,indice) = appartient_c_s_p value.c_s_p value.debut_sp value.fin_sp (id,ne,p) in
	    if b
        then (
          (*printf "%s" ("on l'a bien trouvé ne vaut " ^ string_of_int ne ^"\n") ;*)
          (true,middle, middle_ss,indice)
        )
        else 
				(
				 (*printf "%s" ("\n 2 on ne l'a pas trouvé d,f vaut "^ string_of_int d ^" , "^ string_of_int f ^ " et value_middle vaut " ^ string_of_int value.noeud  ^ "\n");*)
...
(**premiere approche**)
let rec get_ident_noeud p =
	match p with 
  | Implie (e1,e2) -> get_ident_noeud e1 ^ "\n" ^ get_ident_noeud e2
  | And list_and -> "il ne devrait plus y avoir de and list"
  | P_Noeud (id,ne,nv,reg,p) -> "id est " ^ string_of_int id ^ " et le noeud est " ^ string_of_int ne
  | Exp_ar( x1,x2,x3,x4,x5,x6,x7) -> "erreur Exp_ar"
  | Exp_Bool (x1,x2,x3,x4,x5,x6) -> "erreur Exp_bool"
  | Exp_string  x -> "erreur Exp_string"
  | Undefined -> "undefined"
  | And2 (e1,e2) -> get_ident_noeud e1 ^ "\n" ^ get_ident_noeud e1 
  | True -> "fail true"
;;

let print_def_fun p =
	let (*(id,noeud)*)res = get_ident_noeud p in
    res
(*	"id : " ^ string_of_int id ^ " et noeud : " string_of_int noeud*)
;;

let rec print_def_fun_from_prop prop_l = 
  match prop_l with
  | [] -> ""
  | h :: t -> print_def_fun h ^ "\n" ^ print_def_fun_from_prop t 
;;

let rec print_def_fun_from_prop_list cfg_l = 
  match cfg_l with  
  | [] -> ""
  | h :: t -> print_def_fun_from_prop h ^ print_def_fun_from_prop_list t
;;
(** fin premiere approche**)
...
let rec creation_def_fun quad cfg pred nb_var=
  ...
  printf "%s" ("b vaut " ^ string_of_bool b ^ "\n" );
	if ( b  )
      ...
      printf "%s" ("on a fini le noeud " ^ string_of_int ne ^ "\n"); 
			)
and  
...
(* on devra ensuite faire les homogeneisation*)
(** debut homogeneisation**)
  printf "%s" ("gros changement mtn \n\n\n\n");
